...
func AddAddress(data structs.Address) (int64, error) {
	...
	// 执行插入操作
	result := gormDB.Table(addressTableName).Create(&data)
	if result.Error != nil {
...
func UpdateAddress(id int, data structs.Address) error {
	...
	// 执行更新操作
	result := gormDB.Table(addressTableName).Where("id = ?", id).Updates(&data)
	if result.Error != nil {
...
func DeleteAddress(id int) error {
	...
	// 执行删除操作
	result := gormDB.Table(addressTableName).Where("id = ?", id).Delete(&structs.Address{})
	if result.Error != nil {
...
func GetAddressDetail(id, uid int) (*structs.Address, error) {
	...
	result := gormDB.Table(addressTableName).Where("id = ? AND uid = ?", id, uid).First(&address)
	if result.Error != nil {
...
func GetDefaultAddress(weid, uid int) (*structs.Address, error) {
	...
	// 查询默认地址
	var address structs.Address
	result := gormDB.Table(addressTableName).Where("weid = ? AND uid = ? AND isDefault = ?", weid, uid, 1).First(&address)
	if result.Error != nil {
		log.Println("获取默认地址失败:", result.Error)
		return nil, result.Error
	}
	log.Printf("获取默认地址成功: ID=%d, 姓名=%s, 电话=%s, 地址=%s 默认:%v", address.ID, address.Name, address.Telephone, address.Address, address.IsDefault)
...
func GetAddressList(weid, uid int) ([]structs.Address, error) {
	...
	result := gormDB.Table(addressTableName).Where("weid = ? AND uid = ?", weid, uid).Order("isDefault desc, id desc").Find(&addresses)
	if result.Error != nil {
...
func CancelOtherDefaultAddress(weid, uid int) error {
	...
	// 取消其他默认地址
	result := gormDB.Table(addressTableName).Where("weid = ? AND uid = ?", weid, uid).Update("isDefault", 0)
	if result.Error != nil {
...
func SetDefaultAddress(id, weid, uid int) error {
	...
	// 设置默认地址
	result := gormDB.Table(addressTableName).Where("id = ?", id).Update("isDefault", 1)
	if result.Error != nil {
